...
- **`v0.8.1`** — `search_class_features` had a latent regression from `v0.7.0`'s own cold-call narrowing fix: `resolveCandidateClasses` narrowed the subclass fan-out on a *substring* match against `className`, but the tool's own filter matches the *composite* class+subclass name it renders (e.g. "Cleric (War Domain)"). A `className` that substring-matched one base class's name — without exactly naming it — narrowed the fan-out to that class alone and silently dropped every other class's matching subclass features (e.g. `className: "War"` returned only Warlock, never surfacing Cleric's War Domain). `resolveCandidateClasses` now takes an `exactOnly` flag; the cold-call narrowing path uses it, so a substring `className` falls back to the full scan instead of guessing wrong. `getSubclass`/`searchSubclasses` keep the old substring-fallback behavior, since `className` there only ever means "a base class name" and doesn't share the composite-name mismatch this bug depends on. Reported by dmjohnston89 in [PR #12](https://github.com/dmjohnston89/dndbeyond-mcp/pull/12) review; fixed here and verified live against a real account (Cleric's War Domain "War Priest" feature) via an isolated test-agent dispatch.
