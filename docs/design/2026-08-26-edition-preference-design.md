...
`#12` makes classes, subclasses, species, backgrounds, feats, and (via `2026-08-26-pr12-review-response.md`'s
...
- **#2** is the actual fix: a `preferences.edition` block in `~/.dndbeyond-mcp/config.json`, same file and same
  ...
  mechanism, not two ad hoc ones. Precedence: **explicit per-call `edition` param → config per-entity override →
  config global default → hardcoded `"2024"`.**
...
proposed in B3: `DNDBEYOND_EDITION_DEFAULT`, and a single JSON blob (`DNDBEYOND_EDITION_OVERRIDES`) for the
per-entity map, rather than one env var per entity type.
